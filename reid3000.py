    ...
    def evaluate_query(self, query):
        ...
        with torch.no_grad():
            ...
            qf = torch.nn.functional.normalize(qf, dim=1,
                                               p=2)  # TODO probably dangerous to normalize since we have few elemtns, better to do it together with the gallery but more comexp
            ...
            for ass in range(min(m, n)):
                ...
                if distmat[idx] == np.inf:
                    ...
                    self.gallery.update(gf_id[idx[1]], qf[idx[0]]) if self.save_path is None else self.gallery.update(
                        gf_id[idx[1]], qf[idx[0]], query_pics[idx[0]])
